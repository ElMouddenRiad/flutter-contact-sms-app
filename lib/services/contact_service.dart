...
class ContactService extends ChangeNotifier {
    ...
    try {
      ...
      if (snapshot.snapshot.value == null) {
        ...
        if (deviceContacts.isNotEmpty) {
          ...
          await _dbRef.child('users/$userId').update({'contacts': {}});
        }
      }
    } catch (e) {
    ...
    try {
      ...
      for (final contact in contacts) {
        await contactsRef.child(contact.identifier!).set({
          ...
          'phones':
              contact.phones
                  ?.map((phone) => {'value': phone.value, 'label': phone.label})
                  .toList(),
          'emails':
              contact.emails
                  ?.map((email) => {'value': email.value, 'label': email.label})
                  .toList(),
  ...
  Future<List<app_models.Contact>> _getContactsFromFirebase(
    String userId,
  ) async {
    try {
      print("Fetching contacts from Firebase for user: $userId");
      final snapshot =
          await _firebaseService.getUserRef(userId).child('contacts').once();
  ...
  Future<List<app_models.Contact>> restoreContactsFromFirebase(
    String userId,
  ) async {
    try {
      ...
      final newContacts =
          cloudContacts.where((cloudContact) {
            return !deviceContacts.any(
              (deviceContact) => _areContactsSame(deviceContact, cloudContact),
            );
          }).toList();
  ...
  Future<void> syncContacts(String userId) async {
    ...
    final contactsToUpdate =
        deviceContacts.where((deviceContact) {
  ...
  Future<bool> writeContactsToDevice(List<app_models.Contact> contacts) async {
    try {
      ...
      print(
        "Successfully wrote $successCount out of ${contacts.length} contacts",
      );
  ...
  bool _areContactsSame(app_models.Contact a, app_models.Contact b) {
    ...
    final aPrimaryPhone =
        a.phones?.isNotEmpty == true ? a.phones?.first.value : null;
    final bPrimaryPhone =
        b.phones?.isNotEmpty == true ? b.phones?.first.value : null;

    return a.displayName == b.displayName && aPrimaryPhone == bPrimaryPhone;
  }
