...
class BackupRestoreView extends StatelessWidget {
  ...
  Future<void> _backupSms(BuildContext context) async {
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Starting SMS backup...')));
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('${messages.length} messages backed up successfully'),
        ),
      );
    } catch (e) {
      if (!context.mounted) return;
  ...
  Future<void> _restoreContacts(BuildContext context) async {
    try {
      ...
      await contactService.restoreContactsFromFirebase(userId);
      ...
      final newContactsCount =
          deviceContactsAfter.length - deviceContactsBefore.length;

      if (newContactsCount > 0) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Successfully restored $newContactsCount contacts'),
          ),
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _restoreSms(BuildContext context) async {
    try {
      ...
      if (!success) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Could not write to device SMS storage'),
          ),
        );
      }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('${messages.length} messages restored successfully'),
        ),
      );
    } catch (e) {
      if (!context.mounted) return;
