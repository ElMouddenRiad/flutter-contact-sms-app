...
class _ContactsViewState extends State<ContactsView> {
        ...
        bool shouldRequestAgain = await showDialog(
          ...
          builder:
              (context) => AlertDialog(
                ...
                content: const Text(
                  "We need access to your contacts to backup them securely",
                ),
                actions: [
                  TextButton(
  ...
  Future<void> _backupContacts() async {
    try {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Backing up contacts...')));
      ...
      await contactService.backupContactsToFirebase(userId, _contacts);
      if (!mounted) return;
