...
class _HomeViewState extends State<HomeView> {
  ...
  Future<void> _initializeData() async {
    ...
    if (userId != null) {
      try {
        await contactService.initializeUserData(userId);
        await smsService.initializeUserData(userId);

        await contactService.syncContacts(userId);
        await smsService.syncSms(userId);
      } catch (e) {
        if (mounted) {
